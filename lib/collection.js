...
var generateName = function(keys) {
  var names = [];
...
class Collection {
  ...
  createIndex(keys, options) {
    ...
    var index = {key: keys}
    for(var n in options) index[n] = options[n];
  ...
  dropIndex(key) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var indexName = typeof key == 'string' ? key : generateName(key);
      // Drop the index
      self.topology.command(f('%s.$cmd', self.dbName), {
  ...
  listIndexes() {
    ...
    // Build the command
    var command = { listIndexes : this.collectionName};
    // Return an aggregation cursor    
    return this.topology.cursor(this.ns, command, options);    
  }
